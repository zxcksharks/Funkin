...
class PlayState extends MusicBeatSubState
{
  ...
  /**
   * The last registered key release input, used in hold note score calculations
   */
   ...
   * The logic for this was moved to `scoring.SongScore`,
   * but we keep this here for partial backwards compatibility.
   */
  public var songScore(get, never):Float;

  function get_songScore():Float
  {
    return SongScore.instance.getScoreWithPending();
  }
  ...
  function processInputQueue():Void
  {
    ...
    {
      // We use `pop()` instead of `shift()` so that lastReleased gets set to the earliest release.
  ...
  function goodNoteHoldRelease(holdNote:SustainTrail, ?input:PreciseInputEvent = null):Void
  {
    ...
    else
    {
      ...
      if (remainingLength > Constants.HOLD_DROP_PENALTY_THRESHOLD_MS)
      {
        ...
        trace(
          'Penalizing score by ${event.score} and health by ${event.healthChange} for dropping hold note (is combo break: ${event.isComboBreak}, was hit: ${holdNote.hitNote})!'
        );
        applyScore(event.score, '', event.healthChange, event.isComboBreak);
